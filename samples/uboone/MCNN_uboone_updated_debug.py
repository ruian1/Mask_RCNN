...
if len(sys.argv) == 5:
    ...
    print sys.argv    
    print "training sample, ", path_train
    print "validation sample, ", path_val
    print "trained weights, ", path_model
...
class UbooneConfig(Config):
    ...
    '''
    # More anchors
    RPN_ANCHOR_RATIOS = [1./16, 1./8, 1./4, 1./2, 1, 2, 4, 8, 16]
    RPN_ANCHOR_SCALES = (16., 24., 32., 48., 64., 96., 128.,192, 256.)  # anchor side in pixels
    BACKBONE_STRIDES =  [4]* len(RPN_ANCHOR_SCALES)
    '''
    # Default anchors
    RPN_ANCHOR_RATIOS = [0.25, 0.5, 1, 2, 4]
    RPN_ANCHOR_SCALES = (16, 32, 64, 128, 192, 256)  # anchor side in pixels
    BACKBONE_STRIDES =  [4, 8, 16, 32, 32, 32, 32]

    # Use smaller anchors because our image and objects are small [width/height]
    #RPN_ANCHOR_RATIOS = [0.25, 0.5, 1, 2, 4]
    #RPN_ANCHOR_RATIOS = [1./8, 1./2, 1,  2, 8]
    #RPN_ANCHOR_RATIOS = np.concatenate((np.arange(0.05,1,0.025), 1./np.arange(0.05,1,0.025)))
    #RPN_ANCHOR_RATIOS = np.concatenate((np.arange(0.05,1,0.05), 1./np.arange(0.05,1,0.05), np.arange(0.01,0.11,0.01), 1./np.arange(0.01,0.11,0.01)))

    #RPN_ANCHOR_SCALES = (16, 32, 64, 128, 192, 256)  # anchor side in pixels
    #RPN_ANCHOR_SCALES = np.arange(4, 256+4, 4) # anchor side in pixels

    #BACKBONE_STRIDES =  [4, 8, 16, 32, 32, 32, 32]
    #BACKBONE_STRIDES =  [4]* RPN_ANCHOR_SCALES.shape[0]
...
class UbooneDataset(utils.Dataset):
    ...
    def load_events(self, count, height, width):
        ...
        for i in range(count):
            ...
            #read image_info for empty images,
            #otherwise image_id and root file idx are mismatched
            #this is handled in data_generator
            else:
                #continue
                pdgs.append(0)
                bbs.append([[0  , 512],
                ...
                self.add_empty_entry(image_id=i)
            #print "pdgs,",pdgs
...
if __name__=="__main__":
    ...
    dataset_val.load_events(9990, config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1])
    #dataset_val.load_events(10, config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1])
    #dataset_val.load_events(1000, config.IMAGE_SHAPE[0], config.IMAGE_SHAPE[1])
    dataset_val.prepare()
    ...
    if sys.argv[1]=='heads_all':
        model_path=path_model
        #model_path="/scratch/ruian/maskrcnn_log/pre-trained/mask_rcnn_coco.h5"
        #model_path="/scratch/ruian/maskrcnn_log/pre-trained/mask_rcnn_coco_2.h5"

        if model_path:
            print "Loading from path, ", model_path
            #model.load_weights(model_path, by_name=True)
            # for mask_rcnn_coco_2.h5
            #model.load_weights(model_path, by_name=True, exclude=["conv1","rpn_model","mrcnn_bbox_fc", \
            #                                                      "mrcnn_class_logits","mrcnn_mask"])


            # for mask_rcnn_coco.h5
            model.load_weights(model_path, by_name=True, exclude=["conv1", "rpn_model","mrcnn_bbox_fc", \
                                                                  "mrcnn_class_logits","mrcnn_mask"])
